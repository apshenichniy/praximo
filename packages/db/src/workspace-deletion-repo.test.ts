...
describe.skipIf(!DATABASE_URL)("WorkspaceDeletionRepo (dev Neon branch)", () => {
  ...
  it.effect("snapshots every owned R2 key, cascades the workspace, and replays the receipt", () =>
    Effect.gen(function* () {
      ...
      yield* repo.markPipeline(requestId, "nothing-active", new Date("2026-07-23T12:02:00.000Z"))
      yield* repo.markFarewell(requestId, "sent", new Date("2026-07-23T12:03:00.000Z"))
      yield* repo.markBotReleased(requestId, "released", new Date("2026-07-23T12:04:00.000Z"))
      const completed = yield* repo.finalize(requestId, new Date("2026-07-23T12:05:00.000Z"))
