...
function InviteCoachPage() {
  ...
  return (
    <main className="mx-auto flex min-h-dvh w-full max-w-2xl flex-col px-5 pt-6 pb-10">
      <HostBackButton onBack={() => void navigate({ to: "/admin" })} />

      <header className="mt-7">
        <Heading as="h1" role="display">
          Invite a coach
        </Heading>
      ...
      <Heading
        as="h2"
        role="card-title"
        className="text-muted-foreground mt-8 font-semibold tracking-widest uppercase"
      >
        Send the invite
      </Heading>
      ...
      {pending ? (
        <p className="text-muted-foreground mt-4 flex items-center gap-2 text-base leading-relaxed">
          <Spinner /> Preparing the invite…
        </p>
      ) : null}

      <p className="text-muted-foreground mt-auto border-t pt-4 text-xs leading-normal leading-5">
...
function InviteAction({
  ...
  return (
    <Item
      ...
      className="transition-colors duration-100 active:bg-muted hover:bg-muted min-h-[70px] w-full rounded-none border-0 text-left transition-colors disabled:opacity-60"
    >
      <ItemMedia>
        <span className="border-primary/30 text-primary flex size-11 items-center justify-center rounded-full border">
          {icon}
        </span>
      </ItemMedia>
