  ...
  Effect.gen(function* () {
    ...
    const layer = Layer.succeed(
      ...
      WorkspaceDeletionRepo.Service.of({
        ...
        prepare: Effect.fn("WorkspaceDeletionRepo.Double.prepare")(
        ...
        isDeleting: Effect.fn("WorkspaceDeletionRepo.Double.isDeleting")(() =>
          Effect.succeed(true),
        ),
  ...
  it.effect("lets a single attempt drive a contested operation, and only that one", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.gen(function* () {
        ...
        const attempt = () =>
          admin
            .deleteWorkspace("valid", workspaceId, { requestId, confirmationName: "Ada Coaching" })
            .pipe(Effect.result)
