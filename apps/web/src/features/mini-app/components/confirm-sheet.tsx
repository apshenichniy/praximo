...
export function ConfirmSheet({
  ...
  const confirm = destructive ? (
    ...
    <Button size="lg" className="h-13 w-full text-emphasis font-semibold" onClick={onConfirm}>
      {confirmLabel}
    </Button>
  )
      ...
      <DrawerContent className={sheetPadding}>
        <DrawerHeader className="p-0 pt-2 text-left group-data-[swipe-axis=y]/drawer-popup:text-left">
          ...
          {description === undefined ? null : (
            <DrawerDescription className="text-body leading-5">{description}</DrawerDescription>
          )}
        </DrawerHeader>
