...
const read = async (): Promise<LaunchCredential> => {
  ...
  // The Coach app always verifies a Workspace/Coach Bot launch. A local browser
  // therefore needs the same `?b=<bot-id>` selector as the real Mini App URL;
  // no selector means no development credential, never an admin identity.
  if (import.meta.env.DEV) {
    const minted = await loadDevelopmentCoachInitData({ data: { botId } }).catch(() => "")
