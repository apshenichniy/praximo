...
export function DayStrip({
   ...
   * fortnight it crawls. This is the `--ease-out` shape — off fast,
    ...
    >
      {days.map((day) => {
        ...
        return (
          ...
          >
            <span className="text-xs leading-normal font-semibold tracking-wide uppercase">
              {weekdayFormat.format(day)}
            </span>
            <span className="text-base leading-snug font-semibold tabular-nums">
      ...
      <button
        type="button"
        onClick={onOpenMonth}
        className="border-border text-muted-foreground ease-[var(--ease-out)] flex w-16 flex-none flex-col items-center justify-center gap-1 rounded-2xl border border-dashed py-2 transition-[color,background-color,border-color,scale] duration-100 active:scale-[0.97]"
      >
        <HugeiconsIcon icon={Calendar03Icon} size={16} strokeWidth={2} />
        <span className="text-xs leading-normal font-semibold uppercase">{monthLabel}</span>
      </button>
    </div>
  )
}
