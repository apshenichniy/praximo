...
describe("legalUrl", () => {
  it("addresses each text on the client app, in the language asked for", () => {
    expect(legalUrl(ORIGIN, "privacy", "ru")).toBe("https://me.praximo.io/legal/privacy?lang=ru")
    expect(legalUrl(ORIGIN, "terms", "uk")).toBe("https://me.praximo.io/legal/terms?lang=uk")
  })

  it("does not double the slash when the origin carries one", () => {
    expect(legalUrl("https://me.praximo.io/", "terms", "en")).toBe(
      "https://me.praximo.io/legal/terms?lang=en",
    )
  })
  ...
  it("drops whatever the origin was carrying", () => {
    expect(legalUrl("https://me.praximo.io/?b=9100777#x", "privacy", "ru")).toBe(
...
describe("legalRedirect", () => {
  it("permanently redirects to the client app", () => {
    ...
    expect(location(response)).toBe("https://me.praximo.io/legal/privacy?lang=en")
  ...
  it("carries the language across", () => {
    for (const language of ["ru", "uk", "en"]) {
      ...
      expect(location(response)).toBe(`https://me.praximo.io/legal/terms?lang=${language}`)
    }
  })
  ...
  it("narrows a language it does not speak to the floor", () => {
    ...
    expect(location(response)).toBe("https://me.praximo.io/legal/terms?lang=en")
  ...
  it("says so rather than looping when no origin is configured", () => {
    for (const origin of ["", "not a url", "ftp://me.praximo.io"]) {
      const response = legalRedirect(origin, "privacy", "https://app.praximo.io/legal/privacy")
