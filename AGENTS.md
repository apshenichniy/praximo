...
Assets. Twelve private `@praximo/*` packages live in `packages/`, including the
...
- **Server-only modules in the Start apps say so, in the file.** A module that holds a secret, a binding or a credential-derived key carries `import "@tanstack/react-start/server-only"` as its first line (or is named `*.server.ts`, which does the same automatically). TanStack Start's import protection then _mocks_ a violating import in dev and _fails the production build_ — where an unmarked module leaks into the client bundle silently. Type-only imports are ignored, so `import type { … }` still crosses freely; a value the page genuinely shares with the server belongs in a third module with no dependencies (`apps/client/src/features/invite/google-signal.ts` is the worked example, #59).
