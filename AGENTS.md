...
bun workspaces + Turborepo. Four deployable Workers in `apps/` (`web`, `client`, `bot`, `pipeline`), eight private `@praximo/*` packages in `packages/`. [ADR 0002](docs/adr/0002-monorepo-layout-and-module-boundaries.md) is the source of truth for the layout and the module boundaries.
...
- **`apps/web` and `apps/client` share a design system, not a package.** The token blocks in both `styles/app.css` files are written by `docs/spec/design-system/apply.py` (one command, six files) and held identical by `apps/client/src/__tests__/design-system-parity.test.ts`. Edit the spec, run the script; never hand-edit a token block. shadcn primitives are copied between the apps rather than shared, deliberately (ADR 0002).
