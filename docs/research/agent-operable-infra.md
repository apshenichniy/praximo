...
| R2 **EU jurisdiction** (`jurisdiction: "eu"`) and `locationHint` | Yes — first-class fields on `POST /accounts/{id}/r2/buckets` | `A` Workers R2 Storage Edit | — |
| Queues (zero in MVP) | Yes | `A` Queues Edit | — |
| Workers routes on `api.praximo.io/*` | Yes | `Z` Workers Routes Edit | — |
| Worker custom domain `app.praximo.io` | Yes — `PUT /accounts/{id}/workers/domains` | `A` Workers Scripts Edit + `Z` Zone Read | Requires an **active zone** (once-ever, already satisfied) |
...
| Email Sending — **sending subdomain** `mail.praximo.io` | **Yes** — `POST /zones/{zone_id}/email/sending/subdomains`; SPF/DKIM/DMARC/`cf-bounce` records auto-created for zones on Cloudflare DNS | `Z` — ⚠️ exact group not published, see [unverified](#claims-not-verified-against-a-primary-source) | — |
...
- **Email Sending subdomain — required token permission.** `POST /zones/{zone_id}/email/sending/subdomains` exists and is zone-scoped, but Cloudflare's published [API token permissions](https://developers.cloudflare.com/fundamentals/api/reference/permissions/) list contains no Email *Sending* group — only `Email Routing Rules Read/Edit/Write` (zone) and `Email Routing Addresses Read/Edit` (account). The most likely candidate is `Email Routing Rules Edit` on the zone, but this is inference. **Resolve empirically during [#13](https://github.com/apshenichniy/praximo/issues/13)** by calling the endpoint with a zone token carrying only that group, or by enumerating `GET /user/tokens/permission_groups` with a real credential (that endpoint requires auth, so it could not be checked here).
- **`Secrets Store Write` as a permission-group name.** Alchemy's tutorial uses this literal string; Cloudflare's Secrets Store [access-control docs](https://developers.cloudflare.com/secrets-store/access-control/) call it **`Account Secrets Store Edit`**. The *capability* is verified against Cloudflare; the exact string Alchemy passes may need adjusting.
- **`Account > API Tokens > Write`.** Alchemy's tutorial states an admin token needs both `User > API Tokens > Write` *and* `Account > API Tokens > Write`. Cloudflare's published permission list shows `API Tokens Read/Edit/Write` under the **User** scope only. Account-owned token creation is instead documented as gated on **Super Administrator** account role.
