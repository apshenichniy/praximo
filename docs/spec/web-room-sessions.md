...
**In-room UI:** LiveKit Components React defaults — mute, camera toggle, device selection, screen share (on; not recorded — audio-only egress), leave; chat off; recording indicator always on; waiting state ("waiting for the other party") while `scheduled`. Coach-only: **End session** and **Extend +15** (visible during grace). No name entry anywhere — identities come from the token; both participants are shown with their domain names and avatars (Telegram profile photos: client's snapshotted just after their acceptance commit, [#231](https://github.com/apshenichniy/praximo/issues/231); coach's at bot provisioning and refreshed by the daily health sweep, [#225](https://github.com/apshenichniy/praximo/issues/225)). UI language per role: client link → `client.language`, coach → coach's `Member.language`. Mobile browsers are supported (clients arrive from Telegram on phones).
