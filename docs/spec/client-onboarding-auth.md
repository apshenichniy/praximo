...
- **Two forms of the same token:** the deep link `t.me/<workspace_bot>?start=inv_<token>` for Telegram delivery, and the web URL `https://my.praximo.io/i/<token>` for everything else — it opens the web acceptance page. Which form to hand out is a delivery-time choice; there is no universal routing page (a deep-link click from a Telegram chat must open the bot instantly).
...
For invites delivered outside Telegram, the same token opens `my.praximo.io/i/<token>` (the `client` Worker — [#191](https://github.com/apshenichniy/praximo/issues/191); the path is `/i/` rather than `/invite/` so the whole URL is 33 characters and fits the manual-client forward template's 256-character `copy_text` ceiling). Deliberately shaped like a familiar sign-up — the client meets standard UX patterns, not a bespoke consent wall:
...
The room runs on `my.praximo.io/room/<token>` — the `client` Worker (#191), for both actors: by [ADR 0006](../adr/0006-coach-authentication-in-mvp.md) the coach's Mini App credential must never reach an external browser, so the coach joins from a browser like anybody else. The route is reserved; the web-room slice fills it.
