...
2. **Profile** — **Continue with Google**, or a name and an email typed by hand. The layout must read as finished either way, because the Google button arrives separately with [#59](https://github.com/apshenichniy/praximo/issues/59).
   ...
   - **No avatar upload.** Nothing displays a client's avatar yet, Google brings a picture for free, and a cropper would cost a dependency, a primitive, a canvas pipeline and a Safari fallback to normalise an image nobody sees. Initials are the fallback everywhere.
   - Google, fact-checked in [#28](https://github.com/apshenichniy/praximo/issues/28): scopes `openid profile email` cover everything; `sub` is always in the ID token, but name and picture are **not guaranteed** there — read the profile from `userinfo`. Read `email_verified` rather than assuming it; it is `false` when a non-Google domain is attached to the account. The `picture` URL is fetched server-side and snapshotted to R2 (googleusercontent URLs are not stable long-term). The button **must follow Google's branding guidelines** to pass OAuth app verification; localizing "Continue with Google" is encouraged. It sits **above** the manual fields so an import cannot discard what the client already typed (validated in the prototype), with a popup GIS flow and a full-page redirect fallback that persists a draft first.
