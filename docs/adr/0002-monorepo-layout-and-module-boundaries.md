...
| `@praximo/storage` | Both ends of the shared R2 bucket for avatars: the key vocabulary, the `AvatarStore` writer, and the `AvatarReader` that serves one back. Its own package because each end has two callers with nothing else in common — `bot` snapshots a coach's Telegram profile photo ([#225](https://github.com/apshenichniy/praximo/issues/225)) and a client's at acceptance ([#231](https://github.com/apshenichniy/praximo/issues/231)), `client` snapshots a Google `picture` on the Acceptance Page ([#59](https://github.com/apshenichniy/praximo/issues/59)); `coach` serves a client's photo authorised by a launch credential and `client` serves the coach's authorised by an invitation token. The key layout, the content types an avatar may be, the size it may not exceed and the caching contract a served one carries must each be one decision rather than two. No Drizzle and no Telegram; the bucket binding arrives as a structurally narrowed interface, and the reader *describes* its response rather than building one, so the package needs neither a DOM lib nor Workers types and does not depend on the runtime it deploys to. Choosing *which* avatar — the authorisation and the scope — stays with the Worker that understands it. Deleting objects stays with `object_cleanup_job` and the Pipeline's `ObjectCleanup`. **`AvatarReader.unwiredLayer` is a deliberate exception to "placeholder layers fail loudly"** (`AGENTS.md`): where `EmailChannel`'s unwired layer refuses a send with a typed error, this one answers 404. An email nobody received is a broken promise; an avatar nobody served is a courtesy withheld, and the initials behind it are the specified design on every surface — so under `vite dev`, which has no bindings to offer, a photoless run is correct rather than pretending. |
