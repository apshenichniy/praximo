...
**Cross-worker communication is service bindings only** (typed `WorkerEntrypoint` RPC, declared in Alchemy): web → pipeline to trigger the brief workflow on session creation; pipeline → bot for artifact delivery and failure notifications. No public HTTP between our own Workers, no queues (per ADR 0001).
