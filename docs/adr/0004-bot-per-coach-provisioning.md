...
3. On `Update.managed_bot` / `ManagedBotCreated`, everything is automatic: `getManagedBotToken` → **tell the coach their bot is being set up** → branding → `setChatMenuButton` pointing at the Mini App → the activation transaction → `setWebhook` with a fresh per-bot secret → **turn that first message into the ready greeting**. No manual steps after the tap.
   - **The coach is told, because they are already waiting.** Telegram ends its creation dialog with a **Start bot** button, so they tap it seconds before there is anything to answer with; several of those seconds are ours (an avatar upload, two descriptions, the menu button, the activation CTE). Silence there reads as failure, and a coach who taps again is how #135's second bot gets created ([#154](https://github.com/apshenichniy/praximo/issues/154)).
