...
- **Managed Bots one-tap provisioning is the primary path.** Manual BotFather
  ...
  - ⚠️ **The launch mechanism matters, verified 2026-07-25.** Creation can be
    ...
    - **Telegram Web does not implement the link at all, verified 2026-07-25**:
      tapping it inside a logged-in `web.telegram.org/k/` opens nothing, and
      `tgaddr=tg://newbot?…` is not routed either. Confirmed in the clients' own
      source — `newbot` appears nowhere in Web K's `internalLink.ts` /
      `internalLinkProcessor.ts` (its creation UI is reachable only from the
      `KeyboardButtonRequestManagedBot` reply-keyboard button, the one iOS
      refuses), and Web A has no managed-bot support at all. Desktop and iOS do
      route it. **So no client offers both entry points**, and a coach who lives
      in the web client has no one-tap path: the BotFather token fallback
      ([#95](https://github.com/apshenichniy/praximo/issues/95)) is their only
      route — which the prompt's own copy already offers them unprompted.
    - The iOS reply-keyboard failure is Telegram's own open bug,
      [bugs.telegram.org/c/61568](https://bugs.telegram.org/c/61568) ("Managed bot
      request button does not work from reply keyboard on iOS"), filed 2026-05-06
      and still open — independent of our finding, and the reason not to wait for
      it to be fixed.
...
2. Manager bot sends one message carrying a `t.me/newbot/{manager}/{suggested}` deep link on an inline `url` button, with a **suggested username derived from the workspace name plus a short tag derived from its id** — the bare stem of a short name (`demo_bot`) is long since registered on Telegram, and the suggestion now rides in a URL the coach reads, while the tag stays stable across the repeated `/start` that re-sends the prompt ([#147](https://github.com/apshenichniy/praximo/issues/147)). The coach edits both in Telegram's own dialog.
   - **The link carries a suggested display name too, and that half is not a
     courtesy.** `bots.createBot` takes a name of 1–64 characters, and
     [api/links](https://core.telegram.org/api/links) says of the link's `name`
     parameter: "Can be empty, in which case the user must choose a name before
     invoking `bots.createBot`". A link without it therefore opens a dialog with
     an empty required field whose **Create** button does not respond until the
     coach notices and fills it in — which reads as a broken button, not as an
     unfinished form. The suggestion is the workspace name, clamped to 64
     characters: the same name the prompt's own text already shows the coach, and
     the same one the username is derived from.
3. On `Update.managed_bot` / `ManagedBotCreated`, everything is automatic: `getManagedBotToken` → `setChatMenuButton` pointing at the Mini App → **tell the coach their bot is being set up** → branding → the activation transaction → **turn that first message into the ready greeting** → `setWebhook` with a fresh per-bot secret. No manual steps after the tap.
   - **The menu button goes on the coach's own chat first, then on the bot's default** ([#156](https://github.com/apshenichniy/praximo/issues/156)). A Telegram client reads a bot's menu button when it opens the chat and caches what it read, and the coach opens theirs by tapping **Start bot** the moment the bot exists — the same moment this update arrives. Setting it at the end of configuration left them with no **Open** button until they reopened the chat: verified by `getChatMenuButton` returning the right `web_app` button while the coach saw none.
     - **Moving it first did not fix that, and the scope is why — verified 2026-07-25.** `setChatMenuButton` without a `chat_id` changes the *default*, and a default is the one thing Telegram cannot deliver: `updateBotMenuButton` names one bot for one recipient, so a change nobody is addressed by reaches a client only when it next refetches the bot. No ordering wins a race against a value that is never pushed, which is why the reorder alone left the coach looking at the same absence. Addressed to the coach's chat — `bots.setBotMenuButton` takes a `user_id` — the same change is an update their client applies in place. **Both are set**: the targeted one for the coach who is waiting, the default for every later reader, including their other devices and their own clients.
     - What makes a residual miss survivable is the greeting's own inline **Open** button, which works regardless. The step also validates `COACH_MINI_APP_URL`, so an unusable one fails before any promise is made.
   ...
   - **`setWebhook` passes `drop_pending_updates` on this path, and only once the greeting has actually been delivered.** The one thing Telegram is holding is that early `/start`, and dropping it is right exactly when we have answered it ourselves — a replay would otherwise greet the coach a second time. But a coach who taps **Start bot** *after* this run began has been greeted by nothing: both sends were refused for want of a chat to write in, and their `/start` is queued precisely because the webhook is being armed only now. Dropping it there leaves them looking at an empty chat whose only way in is the menu button; delivered, it reaches the bot's own `/start` handler and the same **Open** button. So the drop is conditional on the greeting having landed — one greeting too many is the affordable error, none at all is not. Never on the BotFather path (#95), where a queued update may be the proof handshake itself, and never when re-configuring an installed bot, whose queue is the coach's and their clients' ordinary messages.
...
- One `bot` Worker serves all bots, path-routed: `api.praximo.io/telegram/{bot_id}` per coach bot, `api.praximo.io/telegram/manager` for the manager bot.
- **`allowed_updates` is a subscription, not a filter, and it is set by `setWebhook` rather than by a deploy.** An update type left off the list is one Telegram never sends: the manager bot subscribes to `message`, `callback_query` and `managed_bot`, coach bots to `message` and `callback_query`. A handler added for a type not on the list looks exactly like a handler that does not work — the coach taps a button, their client spins, and nothing reaches the Worker ([#164](https://github.com/apshenichniy/praximo/issues/164), where the creation prompt's second button landed before `callback_query` did). Changing the manager's list means running `bun run manager-bot:set-webhook` against every stage; coach bots pick theirs up from the arming call on their next configuration.
...
- **In-chat menu button** (`setChatMenuButton`, `web_app`) — an ordinary Bot API call, set **programmatically** with the bot's token: on the manager bot at setup (`scripts/set-menu-button.ts`, [#80](https://github.com/apshenichniy/praximo/issues/80)) and on each coach bot at provisioning (step 3 above), labelled `"Open"`. On a coach bot it is set **twice — once addressed to the coach's chat, once as the default** — because only the addressed form is delivered to a client that has already read the bot (see step 3).
...
- Facts the research could not verify from primary sources (exact `KeyboardButtonRequestManagedBot` field list, `replaceManagedBotToken` parameters, any cap on bots per manager bot) are checked empirically during the bot Worker's implementation spike; none of them gate this decision.
  - **"Any cap on bots per manager bot" is answered, 2026-07-25: there is none.** [api/bots/managed-bots](https://core.telegram.org/api/bots/managed-bots) puts the only documented ceiling on the **creating user** — `BOT_CREATE_LIMIT_EXCEEDED` "if the current user already owns the maximum allowed number of owned bots", sized by the `bots_create_limit_default` / `bots_create_limit_premium` config keys (20 and 40). Nothing is counted against the manager bot, so a manager bot does not wear out; and a coach at their own ceiling is a coach Telegram tells to buy Premium or to delete a bot in @BotFather. That limit is invisible to us — no Bot API method lists or counts a user's owned bots — so it can only ever be diagnosed from the coach's side, which is what makes the BotFather token fallback (#95) the answer to it rather than a retry.
  - **There is also an undocumented per-user *rate* limit on creation itself, and `bots.createBot` answers it with `FLOOD_WAIT_X`.** Not in that method's documented error list, but every client that implements the feature handles it explicitly, which is what establishes it: Android branches on `err.text.startsWith("FLOOD_WAIT_")` and shows `CreateManagedBotLimitTextTime` — *"You can create another bot in %s."* — a string it keeps **separate from** the count-based `CreateManagedBotLimitText` ("You can create up to %d bots…"); Telegram Desktop calls `.handleFloodErrors()` and toasts `lng_flood_error`; Telegram Web K's `createManagedBot` returns `{status: 'wait', waitTime}` from `getFloodWaitTime`. No config key exposes it — `help.getAppConfig` carries only `bots_create_limit_default` / `bots_create_limit_premium` — so it can only ever arrive as a raw `FLOOD_WAIT`.
    - **Observed 2026-07-25**: one test identity created four managed bots over six hours and was then refused every further attempt, while a second identity created bots through the same manager bot in the same minute. On the refused account `bots.createBot` was still *sent* — seen in the client's own MTProto log — `@BotFather`'s `/newbot` still worked, and `/mybots` listed nothing (every bot had been deleted), so it was neither the owned-bots ceiling nor anything about the username, the name, or the manager bot. **No public source states the window's size**; this observation appears to be the only datapoint that exists.
    - **On iOS and macOS the refusal is invisible by construction, which is why the coach sees a Create button that does nothing.** `_internal_createBot` maps only `USERNAME_OCCUPIED` and `BOT_CREATE_LIMIT_EXCEEDED` and calls `network.request` with its default `automaticFloodWait: true`, so MTProto swallows the flood error and retries internally — the signal never emits and never fails, and the sheet stays mid-creation forever. The macOS client shares that code (its log paths run through `telegrammacos/submodules/telegram-ios/submodules/TelegramCore`). Android and Desktop escape it only because they opt out explicitly. **A coach reporting a dead Create button is therefore reporting a rate limit until proven otherwise** — and it is worth asking them to retry on Android, the one client that shows the remaining wait.
    - The consequence for the product is that a coach can lock themselves out of one-tap creation by retrying onboarding a few times, and neither they nor we are told why. The prompt's standing offer to accept a @BotFather token is what carries them through it — `/newbot` is on a different path and keeps working.
